...
public class SimdBitPacking2 {
  ...
  static void simdPack(int[] input, byte[] output, int bit) {
    assert input.length == 128;
    byte[] asBytes = new byte[128];
    for (int i = 0; i < 128; i++) {
      asBytes[i] = (byte) (input[i] & 0xFF);
    }

//    for (int off = 0; off < 128; off += 4) {
//      IntVector in = IntVector.fromArray(SPECIES_128, input, off);
//      in.convertShape(VectorOperators.I2B, BYTE_SPECIES_128, 0).reinterpretAsBytes().intoArray(asBytes, off);
//    }
...
//    IntVector in = IntVector.fromArray(SPECIES_128, input, off);
//    in.convertShape(VectorOperators.I2B, BYTE_SPECIES_128, 0).reinterpretAsBytes().intoArray(asBytes, off);
//    in = IntVector.fromArray(SPECIES_128, input, off+=4);
//    in.convertShape(VectorOperators.I2B, BYTE_SPECIES_128, 0).reinterpretAsBytes().intoArray(asBytes, off);
//    in = IntVector.fromArray(SPECIES_128, input, off+=4);
//    in.convertShape(VectorOperators.I2B, BYTE_SPECIES_128, 0).reinterpretAsBytes().intoArray(asBytes, off);
//    in = IntVector.fromArray(SPECIES_128, input, off+=4);
...
//    in.convertShape(VectorOperators.I2B, BYTE_SPECIES_128, 0).reinterpretAsBytes().intoArray(asBytes, off);
    ...
    switch (bit) {
      case 1: return;
      case 2: SIMD_fastPack2(asBytes, output); return;
      case 3: return;
      case 4: return;
      case 5: return;
      // TODO 6 .. 32
  ...
  static void SIMD_fastPack2(byte[] input, byte[] output) {
    int inOff = 0;
    int outOff = 0;
    ByteVector outVec;
    ...
    outVec = inVec;
    inVec = ByteVector.fromArray(BYTE_SPECIES_128, input, inOff+=16);
